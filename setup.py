...
"""
    ...
    parser.add_argument("project", nargs="*", help="Path to .uproject file (quote if contains spaces)")
    parser.add_argument("--index", action="store_true", help="Build semantic index after setup")
    ...
    unrealagent_dir = script_dir / "UnrealAgent"
    ...
    run_command(["dotnet", "build", "-c", "Release", "--verbosity", "quiet"], cwd=uassetapi_dir)
    ...
    if plat == "windows":
        run_command(["dotnet", "build", "-c", "Release", "--verbosity", "quiet"], cwd=assetparser_dir)
        parser_path = assetparser_dir / "bin" / "Release" / "net8.0" / f"AssetParser{exe_ext}"
    else:
        # macOS/Linux: build self-contained to avoid runtime dependency
        run_command(["dotnet", "publish", "-c", "Release", "-r", rid, "--self-contained", "true", "--verbosity", "quiet"], cwd=assetparser_dir)
        parser_path = assetparser_dir / "bin" / "Release" / "net8.0" / rid / "publish" / f"AssetParser{exe_ext}"
    ...
    run_command([sys.executable, "-m", "pip", "install", "-q", "-r", str(requirements_file)], check=False)
    ...
    if args.index:
        ...
        if not project_path:
            ...
            result = run_command(
                [sys.executable, "tools.py", "--index", "--mode", "full"],
                cwd=unrealagent_dir,
                check=False
            )
    ...
    if project_name:
        ...
        print()
        print("Quick test:")
        print('  cd UnrealAgent')
        print('  python -c "import tools; print(tools.list_assets(\'/Game\'))"')
        print()
