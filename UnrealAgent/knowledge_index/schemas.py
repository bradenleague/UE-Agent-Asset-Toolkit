...
class DocChunk:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "references_out": json.dumps(self.references_out) if self.references_out else "[]",
            "module": self.module,
...
class MaterialParamsDoc(DocChunk):
    ...
    def __init__(
        ...
        text_parts.append(f"Domain: {domain}, Blend: {blend_mode}, Shading: {shading_model}")
...
class MaterialFunctionDoc(DocChunk):
    ...
    def __init__(
        ...
        metadata = {
            ...
            "param_count": len(scalar_params) + len(vector_params) + len(static_switches),
        }
...
class SourceFileDoc(DocChunk):
    ...
    def __init__(
        ...
        text_parts = [f"{name} is a {'header' if name.endswith('.h') else 'source'} file"]

        if line_count > 0:
            text_parts.append(f"with {line_count} lines")

        if class_count > 0:
            text_parts.append(f"containing {class_count} class{'es' if class_count > 1 else ''}")

        if function_count > 0:
            text_parts.append(f"{function_count} function{'s' if function_count > 1 else ''}")

        if property_count > 0:
            text_parts.append(f"{property_count} propert{'ies' if property_count > 1 else 'y'}")
