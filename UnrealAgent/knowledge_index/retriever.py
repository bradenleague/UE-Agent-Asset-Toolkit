...
class LinkResolver:
    """
    Resolves doc IDs to canonical external URLs.

    Supports multiple link targets:
    - GitHub
    - Bitbucket
    - Perforce
    - Confluence
    - Unreal Editor deep links
    """
    ...
    def __init__(self, config: dict = None):
        ...
        Args:
            config: Configuration dict with URL templates and settings
        """
        self.config = config or {}

    def resolve(self, doc_id: str, link_types: list[str] = None) -> dict[str, str]:
        """
        Resolve doc ID to external URLs.

        Args:
            doc_id: Document ID to resolve
            link_types: Types of links to generate (default: all configured)

        Returns:
            Dict mapping link type to URL
        """
        links = {}
        link_types = link_types or list(self.config.keys())

        # Parse doc_id to extract path and type
        doc_type, path = self._parse_doc_id(doc_id)

        for link_type in link_types:
            if link_type in self.config:
                url = self._generate_url(doc_type, path, link_type)
                if url:
                    links[link_type] = url

        return links

    def _parse_doc_id(self, doc_id: str) -> tuple[str, str]:
        """Parse doc_id into type and path."""
        if ":" in doc_id:
            parts = doc_id.split(":", 1)
            return parts[0], parts[1]
        return "unknown", doc_id

    def _generate_url(self, doc_type: str, path: str, link_type: str) -> Optional[str]:
        """Generate URL for a specific link type."""
        template_config = self.config.get(link_type, {})

        if doc_type == "asset":
            template = template_config.get("asset")
        elif doc_type == "cpp":
            template = template_config.get("cpp")
        elif doc_type in ("material", "widget", "bp_func", "datatable"):
            template = template_config.get("asset")
        else:
        ...
        # Handle function references in path
        func_name = ""
        if "::" in path:
            path, func_name = path.rsplit("::", 1)
        ...
        try:
            return template.format(
                ...
                project=template_config.get("project", ""),
                domain=template_config.get("domain", ""),
                space=template_config.get("space", ""),
            )
        except KeyError:
            return None


# Default link resolver configurations

GITHUB_CONFIG = {
    "org": "",  # Set your org
    "repo": "",  # Set your repo
    "branch": "main",
    "asset": "https://github.com/{org}/{repo}/blob/{branch}/Content{path}.uasset",
    "cpp": "https://github.com/{org}/{repo}/blob/{branch}{path}",
}

PERFORCE_CONFIG = {
    "project": "",  # Set your depot path
    "asset": "p4v://depot/{project}/Content{path}.uasset",
    "cpp": "p4v://depot/{project}{path}",
}

CONFLUENCE_CONFIG = {
    "domain": "",  # Set your Confluence domain
    "space": "",  # Set your space key
    "asset": "https://{domain}/wiki/spaces/{space}/pages?title={path}",
}
