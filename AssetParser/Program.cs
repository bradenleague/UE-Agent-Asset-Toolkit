...
string DetectAssetType(string path, List<string> exportClasses)
{
    ...
    foreach (var (prefix, type) in NamingPrefixes)
    {
    ...
    if (parentClass == "Unknown" || parentClass == "[null]")
    {
        ...
        foreach (var import in asset.Imports)
        {
            ...
            if (importClass == "BlueprintGeneratedClass" && importName.EndsWith("_C"))
            {
                ...
                // Prefer Lyra/HUD/Layout classes as they're more likely to be parents
                if (baseName.Contains("HUD") || baseName.Contains("Layout") ||
                    baseName.Contains("Activatable") || baseName.StartsWith("Lyra"))
                {
    ...
    for (int i = 0; i < asset.Exports.Count; i++)
    {
        ...
        if (export is NormalExport normalExport && normalExport.Data != null)
        {
            foreach (var prop in normalExport.Data)
            {
                ...
                {
                    if (prop is TextPropertyData textProp && textProp.Value != null)
                    {
                        var textVal = textProp.Value.ToString();
                        // Skip localization keys (32-char hex strings)
                        if (!string.IsNullOrEmpty(textVal) && textVal.Length < 32)
                            props["text"] = textVal;
                    }
                }
                // Extract visibility
...
object GetPropertyValue(PropertyData prop, int depth)
{
    ...
    try
    {
        return prop switch
        {
            ...
            ObjectPropertyData objProp => objProp.Value.Index,
        ...
        try
        {
            ...
            for (int i = 0; i < asset.Exports.Count; i++)
            {
                ...
                if (export is NormalExport normalExport && normalExport.Data != null)
                {
                    foreach (var prop in normalExport.Data)
                    {
                        if (prop.Name.ToString() == "Text" && prop is TextPropertyData textProp && textProp.Value != null)
                        {
                            var textVal = textProp.Value.ToString();
                            if (!string.IsNullOrEmpty(textVal) && textVal.Length < 32)
                                textContent = textVal;
