...
class TestInspectAssetDetailRouting:
    @patch("unreal_agent.assets.inspector._asset_path_to_file")
    @patch("unreal_agent.assets.inspector._run_asset_parser")
    def test_graph_detail_routes_to_graph_plus_json_command(
        self, mock_run, mock_to_file
    ):
        mock_to_file.return_value = "/tmp/BP_Test.uasset"
        mock_run.return_value = "{}"

        inspect_asset("/Game/Test/BP_Test", summarize=True, detail="graph")

        mock_run.assert_called_once_with("graph-plus-json", "/tmp/BP_Test.uasset")

    @patch("unreal_agent.assets.inspector._asset_path_to_file")
    @patch("unreal_agent.assets.inspector._run_asset_parser")
    def test_graph_summary_detail_routes_to_graph_summary_json_command(
        self, mock_run, mock_to_file
    ):
        mock_to_file.return_value = "/tmp/BP_Test.uasset"
        mock_run.return_value = "{}"

        inspect_asset("/Game/Test/BP_Test", summarize=True, detail="graph-summary")

        mock_run.assert_called_once_with("graph-summary-json", "/tmp/BP_Test.uasset")
