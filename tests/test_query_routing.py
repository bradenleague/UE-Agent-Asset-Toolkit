...
def test_mcp_inspect_asset_tool_schema_graph_enum():
    tools = asyncio.run(mcp_server.list_tools())
    inspect_tool = next(tool for tool in tools if tool.name == "inspect_asset")
    detail_schema = inspect_tool.inputSchema["properties"]["detail"]
    assert detail_schema["enum"] == ["graph", "graph-summary"]
